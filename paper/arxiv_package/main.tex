...
\small \url{https://github.com/shamiquekhan/PINN-Mechanistic-Interpretability} (v3.2.0)}
\date{September 2026}
...
matched controls ($\ET = +0.0020$, 95\% CI $[-0.0034, +0.0084]$; 0/8 features
...
\emph{suggestive} asymmetry --- 6/8 features survive multiple-comparison
...
preregistered decision rules. Practically, conventional signals (gradients,
residuals) drive an early-warning monitor (AUROC 0.859) and a closed-loop
controller that rescues the boundary-starvation failure 18$\times$ over
no-action, with SAE features measured as inert cargo. We recommend a
...
\begin{enumerate}
  ...
  \item \textbf{A measured regime boundary, both sides.} A Fourier neural
        ...
        6/8 MC-corrected survivors where every PINN basis had 0/8 --- a
        suggestive causal asymmetry we report with its caveats.
  \item \textbf{An engineering alternative that works.} Conventional
        signals (gradient statistics, residual spectra) drive a monitor
        with AUROC 0.859 and a bounded closed-loop controller that rescues
        boundary starvation 18$\times$ over no-action --- with a
        monitor-source ablation proving the SAE features are inert cargo
        in the loop.
\end{enumerate}
...
\cite{wang2020understanding,krishnapriyan2021characterizing}, spectral bias
toward low frequencies, and collocation starvation; adaptive remedies include
GradNorm \cite{chen2018gradnorm}, NTK-based weighting
\cite{wang2021learning}, and residual-based attention
\cite{anagnostopoulos2024residual}. We compare all three against our
controller on the same failure (\S\ref{sec:sota}).

\paragraph{Superposition and SAEs.} The superposition hypothesis
\cite{elbage2022toy} holds that networks store more features than neurons by
encoding them in non-orthogonal directions; SAEs
...
SAE features, $\ET = +0.0020$ with CI $[-0.0034, +0.0084]$ --- includes
zero; 0/8 features survive Bonferroni \emph{or} BH-FDR; all 88 target
deltas are positive (the representational signature: any active-latent
ablation damages the decode). The identical battery on PCA components:
$\ET = -0.0203$, CI $[-0.0331, -0.0085]$, 0/8 survive. The planted-feature
positive control \emph{passes} (targeted effect 15$\times$ its controls,
decoder cosine 0.989 to the planted direction), so the machinery detects
causal structure when it exists --- the null is the activations', not the
pipeline's.
...
\begin{itemize}
  ...
  \item Condition (i) MET: 6/8 features survive Bonferroni \emph{and}
        BH-FDR, each at 8/8 batch consistency ($p = 0.0039$, the exact
        sign test's floor at $n{=}8$).
  \item Condition (ii) MET: $\ET$ CI $[+7.1\times10^{-6}, +1.6\times10^{-5}]$
        --- excludes zero, positive; the first battery in this campaign to
        do so (PINN: $[-0.0034, +0.0084]$ SAE, $[-0.0331, -0.0085]$ PCA).
...
Comparative context, from the same scoring code: the target beats all
three controls in 48/64 evaluations (75\%) for the FNO battery vs.\ 12/88
(14\%) for SAE-on-PINN and 28/88 (32\%) for PCA-on-PINN; the median
target/control ratio is 1.2$\times$ vs.\ 1.00$\times$ and 0.98$\times$
(Figure~\ref{fig:operator-causal}, Table~\ref{tab:operator-causal}).

We report this as a \emph{suggestive causal asymmetry} across the regime
boundary, deliberately not as a confirmed causal boundary, for three
recorded reasons: (a) absolute effect sizes are tiny ($\sim$10$^{-5}$ on a
near-zero baseline regression loss); (b) the partial interchange does not
beat a random basis in the operator setting either ($-1.257$ vs.\
...
\begin{figure}[t]
  ...
  asymmetry (0/8, 0/8, 6/8 under the same Bonferroni threshold). Right:
  target-beats-all-controls rate.}
  \label{fig:operator-causal}
\end{figure}

\begin{table}[t]\centering\small
\caption{Operator causal battery (stage 14): the survivor asymmetry (6/8 vs 0/8 vs 0/8).}\label{tab:operator-causal}
...
\paragraph{Monitors.} Conventional signals (loss-component gradients,
residual statistics) drive an early-warning monitor with AUROC 0.859
[0.780, 0.952] on a run-level held-out split (leakage-audited); adding SAE
features moves it to 0.864 with P(SAE better) $= 0.53$ --- no established
advantage. The loss-only threshold is near chance (0.468)
...
(IDLE$\to$WARNING$\to$CONFIRMED$\to$ACTING$\to$COOLDOWN) executes bounded
reweighting actions with rollback: it rescues boundary starvation from
0.295 to 0.017 (18$\times$ over no-action; within 1.4$\times$ of the oracle
reweighting). The monitor-source ablation is the punchline: an SAE-feature
monitor and a matched \emph{random}-feature monitor drive bit-identical
rescue trajectories (0.0236 both) --- the SAE features are inert cargo in
the closed loop.

\paragraph{SOTA baselines.}
\label{sec:sota}
On the same failure, NTK-adaptive weighting \cite{wang2021learning} reaches
0.0064 --- better than both our controller (0.0166) and the oracle;
GradNorm \cite{chen2018gradnorm} (2.01) and RBA
\cite{anagnostopoulos2024residual} (2.19) actively harm. The honest
position: the controller is failure-agnostic machinery with rollback ---
not optimization SOTA --- and the interpretability claims are kept
structurally separate from the engineering claims
(Figure~\ref{fig:controller}).
...
\begin{itemize}
  ...
  \item \textbf{The operator causal result is suggestive, not confirmed.}
        The preregistered conjunctive rule fired the null; the survivor
        asymmetry (6/8 vs.\ 0/8) is real but the effect sizes are tiny,
        the interchange is null, and the sign condition is documented as
        mis-designed for ablation batteries. The corrected criterion is
        preregistered for future work.
...
regime boundary are released for exactly that purpose.
...
any results document cites a number ungrounded in an artifact. 146 unit
