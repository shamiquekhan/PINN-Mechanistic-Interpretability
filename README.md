...
9. **Statistical closure (v4.3, R7/R8/R7+):** preregistered TOST equivalence testing records the SAE−PCA difference as **inconclusive at δ=0.01** (point estimate inside the margin, CI straddling — equivalence formally *not* established; never conflated with "no difference detected"); hierarchical analysis answers the pseudoreplication objection with measurement — ICC 0.19, effective n 37.6/88, SE inflation 1.46×; **the core nulls are cluster-robust, one row-level reading (PCA's negative-side CI) does not survive clustering and is reworded everywhere**. The power-upgraded replication (R7+, 20 new seeds through the identical batteries, 31 combined checkpoints) stays inconclusive at 0.38 achieved power — **and shows the difference's sign is not stable across replication** (both readings sub-margin). Formal equivalence remains unestablished at this n — the R7+ full-power extension (163 checkpoints) is a registered statistical follow-up, not required for the core experimental conclusions.
10. **Three-family external validity (v4.3, R9 + R5):** the within-PINN boundary replicates on reaction–diffusion (Fourier PR 4.3–6.0 vs tanh twin ~1.8; SAE beats PCA 28.5–32.8× every seed) as it did on Burgers and Poisson — steady-linear, time-dependent-nonlinear, and steady-stiff families all show the same geometry→compression boundary.
11. **Geometry gates but does not continuously predict (v4.3, R10):** every rank statistic correlates with the SAE-over-PCA advantage (Spearman 0.66–0.83, cluster CIs exclude zero) but none predicts its magnitude continuously (leave-one-arm-out CV R² all negative) — **the PR family is a validated gate, not a continuous predictor**; the residual is embedding structure beyond the covariance spectrum.
12. **Engineering, honestly bounded (stages 6/7/12 + H17/H19):** conventional monitor AUROC 0.875 — but the honest loss-trajectory floor reaches 0.786 (H19: much of the apparent margin was the rule, not the features); controller rescues boundary starvation to oracle level, **wins its home class** (0.00012 vs NTK 0.0044), never degrades catastrophically across the three-failure battery, and loses to NTK-adaptive elsewhere (H17: robustness, not per-failure SOTA). SAE features are inert cargo throughout.
...
## Research status (v4.3.1 — frozen experimental campaign)

**Core findings:**

1. PINN representation geometry separates low- and high-rank regimes across three PDE families.
2. High-rank regimes show large SAE-over-PCA reconstruction advantages (25–56× on Fourier PINNs, 31–38× on Burgers, 28.5–32.8× on reaction–diffusion).
3. Rank statistics identify the compression regime but do not continuously predict compression advantage (validated gate, not a continuous predictor).
4. No tested feature satisfied the preregistered direction-specific causal criterion in any evaluated regime.
