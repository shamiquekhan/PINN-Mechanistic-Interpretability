...
- [ ] `python -m pytest tests/ -q` → **139 passed** (~12 s, CPU or GPU)
...
## Campaign (stages 2–13 reproduce from committed `runs/` stage-1 artifacts; stage 1 retrains from scratch)
...
- [ ] `--stages 6`: monitors — conventional AUROC ≈ 0.872 [0.795, 0.953]
...
- [ ] `--stages 13`: hardening — MDE 85.7%, P(SAE monitor better) ≈ 0.54

## Figures and tables

- [ ] `bash scripts/generate_figures.sh` → 8 PNGs in `figures/generated/` without error
- [ ] `bash scripts/generate_tables.sh` → LaTeX tables in `paper/tables/` without error
- [ ] Spot-check: Figure 3 (causal null) E_T value matches `runs/causal_intervention_results.json` `causal_strength_ci.mean`
