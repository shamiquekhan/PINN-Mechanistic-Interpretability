...
| PCA causal battery | `--stages 8` | `runs/pca_causal_results.json` | E_T = −0.0131 [−0.0268, −0.0006] (row-level; cluster CI spans zero — R8); 0/8; paired PCA−SAE spans zero |
...
| R8 hierarchical analysis (v4.2) | `python scripts/hierarchical_analysis.py` | `runs/hierarchical_analysis/hierarchical_report.json` | **R8b** — ICC 0.192, DEFF 2.34 (n_eff 37.6/88), SE inflation 1.46×; core nulls cluster-robust (paired diff CI [−0.026, +0.031] still spans zero; SAE negative-side CI [−0.034, −0.003] survives); **PCA negative-side CI does not survive clustering** ([−0.039, +0.008] spans zero) — reworded everywhere per the pre-written rule. R7+ power preregistered: **163 checkpoints for 80% at δ=0.01** |
| R9 reaction–diffusion boundary (v4.2) | `python -m experiments.r9_rd_boundary` | `runs/r9_rd_boundary/r9_report.json` | **R9a** — the boundary replicates on the steady-stiff family: Fourier PR 4.3–6.0 (mean 5.10 > bar 3.0) vs tanh twin 1.7–2.0; SAE beats PCA 28.5–32.8× all seeds (tanh twin: PCA ahead 0.5–0.9×); tangent 1 everywhere; rel L2 ~0.001 (architectural contrast). One pre-run machinery fix recorded (degenerate exact≡0 config → non-degenerate forcing=1, the fresh-campaign §3 bug class). Three-family external validity complete |
| R10 geometry predictors (v4.2) | `python scripts/geometry_predictors.py` | `runs/geometry_predictors/geometry_predictors_report.json` | **R10c refined** — all rank statistics correlate with the advantage (Spearman 0.66–0.83, arm-clustered CIs exclude zero) but none predicts it continuously (leave-one-arm-out CV R² all negative; R3 plateau spans 6–137× at fixed ρ): **the PR family is a validated GATE, not a continuous predictor** — residual is embedding structure beyond the covariance spectrum |
| R7+ equivalence replication (v4.3) | `python -m experiments.r7plus_equivalence` | `runs/r7plus_equivalence/r7plus_report.json` | **R7+b (inconclusive at higher power)** — 20 new seeds through the identical batteries; combined 31 ckpts: mean −0.0049 (sign moved past zero — not stable), cluster SD 0.0419 (−19%), 90% CI [−0.0174, +0.0062] still overlapping δ=0.01; TOST p 0.251; achieved power 0.38/0.80. Formal equivalence requires the full 163-checkpoint battery |
